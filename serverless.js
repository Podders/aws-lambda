...
const outputsList = ['name', 'hash', 'description', 'memory', 'timeout', 'code', 'bucket', 'shims', 'handler', 'runtime', 'env', 'role', 'layer', 'arn', 'region']
...
class AwsLambda extends Component {
    ...
    default (inputs = {}) {
        ...
        this.context.debug(`Starting deployment of lambda $ {
            config.name
        }
        to the $ {
            config.region
        ...
        if (!config.role) {
            this.context.debug(`No role provided
            for lambda $ {
                config.name
            }.`)
        ...
        if (
        config.bucket && config.runtime === 'nodejs10.x' && (await utils.dirExists(path.join(config.code, 'node_modules')))) {
            this.context.debug(`Bucket $ {
                config.bucket
            }
            is provided
            for lambda $ {
            ...
            this.context.debug(`Packaging lambda code from $ {
                config.code
            }.`)
            this.context.debug(`Uploading dependencies as a layer
            for lambda $ {
                config.name
            }.`)
        ...
        const prevLambda = await getLambda({
            lambda, ...config
        })

        if (!prevLambda) {
            if (config.bucket) {
                this.context.debug(`Uploading $ {
                    config.name
                }
                lambda package to bucket $ {
                    config.bucket
                }.`)
            ...
            this.context.debug(`Creating lambda $ {
                config.name
            } in the $ {
                config.region
            }
            ...
            const createResult = await createLambda({
                lambda, ...config
            })
            ...
            if (configChanged(prevLambda, config)) {
                ...
                const updateResult = await updateLambdaConfig({
                    lambda, ...config
                })
        ...
        if (this.state.name && this.state.name !== config.name) {
            ...
            await deleteLambda({
                lambda, name: this.state.name
            })
        }

        this.context.debug(`Successfully deployed lambda $ {
            config.name
        } in the $ {
            config.region
        }
    ...
    async remove() {
        ...
        this.context.debug(`Removing lambda $ {
            name
        }
        from the $ {
            region
        ...
        this.context.debug(`Successfully removed lambda $ {
            name
        }
        from the $ {
            region
